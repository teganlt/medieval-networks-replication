...
df <- df[!is.na(rival_br) & !is.na(rival_ss) & !is.na(mother_id) & mother_id != ""]
df[, EMFP := as.integer(birth <= 1215)]
zs <- function(x) (x - mean(x)) / sd(x)
df[, `:=`(z_rival_br = zs(rival_br), z_rival_ss = zs(rival_ss))]
...
fwrite(rbindlist(res), file.path(OUTDIR, "clean_iv", "reg_band_inference.csv"))
cat("Wrote reg_band_inference.csv\n")
